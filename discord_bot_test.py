...
@bot.event
async def on_ready():
    print(f'{bot.user} 봇이 준비되었습니다! (테스트 모드)')
    ...
    try:
        ...
        if result:
            ...
            if isinstance(channel, discord.TextChannel):
                ...
                await channel.send(f"🎯 **퀴즈 #{quiz_id}** (테스트: 30초 후 답변)\n{quiz_only}\n\n⏰ *30초 후에 정답이 공개됩니다!*")
                ...
                conn.commit()
                print(f"✅ 퀴즈 ID {quiz_id} 전송 완료 - 30초 후 답변 예정 (테스트 모드)")
            else:
...
@tasks.loop(seconds=30)  # 테스트: 30초마다 답변 전송 확인
async def send_answer_task():
    """30초가 지난 퀴즈의 답변을 전송 (테스트 모드)"""
    try:
        if QUIZ_CHANNEL_ID is None:
            print("⚠️ QUIZ_CHANNEL_ID가 설정되지 않았습니다. 환경변수를 확인해주세요.")
            return
        ...
        # 30초가 지났고 아직 답변이 전송되지 않은 퀴즈 찾기 (테스트)
        ...
        for quiz_id, quiz_content in results:
            ...
            if isinstance(channel, discord.TextChannel):
                ...
                await channel.send(f"💡 **퀴즈 #{quiz_id} 정답 공개!** (테스트 완료)\n{answer}")
                ...
                ''', (datetime.now(), quiz_id))
                conn.commit()
                print(f"✅ 퀴즈 ID {quiz_id} 답변 전송 완료 (테스트 모드)")
            else:
                print(f"❌ 채널 ID {QUIZ_CHANNEL_ID}는 텍스트 채널이 아닙니다.")
...
@bot.tree.command(name="테스트퀴즈", description="테스트용 퀴즈를 즉시 전송합니다")
async def test_quiz(interaction: discord.Interaction):
    """테스트용 퀴즈 즉시 전송"""
    try:
        conn = sqlite3.connect('quiz_database.db')
        cursor = conn.cursor()
        ...
        if result:
            ...
            # 퀴즈 전송
            await interaction.response.send_message(f"🎯 **테스트 퀴즈 #{quiz_id}**\n{quiz_only}\n\n⏰ *30초 후에 정답이 공개됩니다!*")
            ...
            ''', (datetime.now(), quiz_id))
            conn.commit()
            print(f"✅ 테스트 퀴즈 ID {quiz_id} 전송 완료")
        else:
            await interaction.response.send_message("❌ 전송할 퀴즈가 없습니다.")
